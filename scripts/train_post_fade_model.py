"""train_post_fade_model.py — Train Truth Social fade (mean-reversion) predictor.

Thesis: after a Trump Truth Social post the market makes an initial 30m move
(measured from next market open). That move sometimes overshoots and partially
reverts. This model predicts whether fading (betting opposite to) the initial
move will be profitable over 2h, 4h, or 1d holding periods.

Timeline (all times relative to post):
  T+0      → Trump posts
  T_open   → next market open (entry for directional AND fade model)
  T_open+30m → initial 30m move observed; fade entry point
  T_open+2h  → 2h hold exits
  T_open+4h  → 4h hold exits
  T_open+1d  → 1d hold exits (next daily close)

Input:  D:/WhaleWatch_Data/post_market_data.parquet  (built by build_post_market_data.py)
Output: models/saved/post_fade_{period}.pkl
...
Two dead-zone filters applied at training time:
  1. |spy_initial_ret| >= OVERSHOOT_MIN  — only fade moves large enough to overshoot
  2. |spy_ret_cont|   >= CONT_DEAD_ZONE  — only train on meaningful continuations

Usage:
    python scripts/train_post_fade_model.py
    python scripts/train_post_fade_model.py --regime
    python scripts/train_post_fade_model.py --overshoot-min 0.003
    python scripts/train_post_fade_model.py --report-only
"""
...
logger = logging.getLogger("train_post_fade")
...
def _build_calibrated_model(
    ...
    base = XGBClassifier(
        n_estimators=200,
        max_depth=3,          # shallower — less data than directional model
        learning_rate=0.05,
        subsample=0.8,
...
def _save_model(
    ...
    filename = f"post_fade_{period}{suffix}.pkl"
...
def _filter_fade(
    ...
    mask = (
        df[lbl_col].notna() &
        df["spy_initial_ret"].notna() &
        (df["spy_initial_ret"].abs() >= overshoot_min) &   # meaningful overshoot
...
def run(
    ...
    if not DATA_PATH.exists():
        logger.error("Dataset not found: %s\nRun build_post_market_data.py first.", DATA_PATH)
        return

    df = pd.read_parquet(DATA_PATH)
    logger.info("Loaded %d posts from %s", len(df), DATA_PATH)

    # Verify fade columns exist
    missing_fade = [f"spy_fade_label_{p}" for p in FADE_PERIODS if f"spy_fade_label_{p}" not in df.columns]
    if missing_fade:
        logger.error(
            "Fade label columns missing: %s\n"
            "Re-run build_post_market_data.py to generate them.", missing_fade,
        )
        return

    if min_keywords > 0 and "keyword_count" in df.columns:
        before = len(df)
        df     = df[df["keyword_count"] >= min_keywords]
        logger.info("Filtered to keyword_count >= %d: %d → %d", min_keywords, before, len(df))

    if "posted_at" in df.columns:
        df["posted_at"] = pd.to_datetime(df["posted_at"], utc=True)
        df = df.sort_values("posted_at").reset_index(drop=True)
        logger.info("Date range: %s → %s",
                    df["posted_at"].min().date(), df["posted_at"].max().date())

    # Train / OOS split
    if "posted_at" in df.columns:
        df_train = df[df["posted_at"] <= TRAIN_CUTOFF].copy()
        df_oos   = df[df["posted_at"] >  TRAIN_CUTOFF].copy()
    ...
    # Summary table
    logger.info("\n--- Post fade model summary ---")
    model_files = sorted(MODELS_DIR.glob("post_fade_*.pkl"))
    results = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train Truth Social post fade (mean-reversion) model"
    )
    parser.add_argument("--overshoot-min", type=float, default=DEFAULT_OVERSHOOT_MIN,
                        help="Minimum |spy_initial_ret| to qualify as a fade candidate "
                             "(default: 0.003 = 0.30%%)")
    ...
    parser.add_argument("--min-keywords", type=int, default=1,
                        help="Min keyword count per post (default: 1)")
